...
from analytics_utils.analytics_utils.analysis_utils import get_ctrl_vals, get_lcms_qc_derivedmetrics_checks, calculate_conversion_C18, calculate_conversion_enantioselectivity_C18, calculate_stats_for_array, calculate_stats_for_dataframe, get_unique_variants_in_dataframe
...
class LcmsC18Analytics:
    ...
    def __init__(self,
        ...
        self.metricsetname = 'LcmsC18'
        self.cols_to_get = {'lcms_detections': lcms_detections_columns_to_get}
        ...
        if hit_filter_dict is not None:
            for metric, thres in hit_filter_dict.items():
                self.hit_filter_metric = metric
                self.hit_filter_thres = thres
        else:
            self.hit_filter_metric = None
    ...
    def get_derived_metrics(self, df):
        ...
        conversion = calculate_conversion_C18(df_wderived_metrics, neg_ctrltype=self.neg_ctrltype)
        df_wderived_metrics.loc[:,'measured_conversion_(r)'] = conversion

        return df_wderived_metrics
    

    def get_qc_metrics(self, df, subconc_init=np.nan):
        
        df_wqc_metrics = df.copy()
        df_wqc_metrics = get_lcms_qc_derivedmetrics_checks(df_wqc_metrics, neg_ctrltype=self.neg_ctrltype, subconc_init=subconc_init)
        
        return df_wqc_metrics
...
#             print(grp, '# of pos ctrl samples:', len(df_ctrl_grp_dict['pos']), '; # of EV ctrl samples:', len(df_ctrl_grp_dict[self.neg_ctrltype]))
            ...
            for ctrl_type in ['pos', self.neg_ctrltype]:
                for metric in metricset:
                    ...
                    # calculate hit rate for activity
                    if self.hit_filter_metric is not None and metric == self.hit_filter_metric: 
                        stats = self.stats.copy() + ['nonhitrate']
                        hit_thres = self.hit_filter_thres
                    else: 
                        stats = self.stats.copy()
                        hit_thres = None
                        
                    CTRL_stats_bygrp = calculate_stats_for_dataframe(df_CTRL_grp, metric, stats_to_get=stats, metrics_dict=CTRL_stats_bygrp,
            ...
            if len(df_grp) > 0:
                ...
                for i, metric in enumerate(metricset):
                    ...
                    if self.hit_filter_metric is not None and metric == self.hit_filter_metric: 
                        ...
                        stats = self.stats.copy()
                        hit_thres = None
                    stats_bygrp = calculate_stats_for_dataframe(df_grp, metric, stats_to_get=stats, 
                                                                metric_prefix=f'{metric}_', metrics_dict=stats_bygrp, hit_thres=hit_thres)
    ...
    def get_library_visualizations(self, df, stats_table_bymetricset, columns_to_plot, groupby='library_barcode', top_n_libraries=100, sort_by=('measured_conversion_(r)', 'median')):
